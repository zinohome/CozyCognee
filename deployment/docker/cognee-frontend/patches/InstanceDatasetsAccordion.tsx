...
export default function InstanceDatasetsAccordion({ onDatasetsChange }: InstanceDatasetsAccordionProps) {
  ...
  useEffect(() => {
    ...
    // CozyCognee Patch: 禁用自动检测 cloud 连接
    // 如果设置了 NEXT_PUBLIC_DISABLE_CLOUD_CHECK=true，则不自动检测 cloud 连接
    // 这样可以避免在本地部署时出现 CloudApiKeyMissingError 错误
    if (process.env.NEXT_PUBLIC_DISABLE_CLOUD_CHECK?.toLowerCase() !== "true") {
